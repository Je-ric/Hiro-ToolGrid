// UUID v4 using crypto.randomUUID or manual fallback
...
export function generateToken({ length = 32, upper = true, lower = true, digits = true, symbols = false }) {
  let pool = ''
...
// SHA hashing via Web Crypto
async function sha(algo, text) {
  const buf = await crypto.subtle.digest(algo, new TextEncoder().encode(text))
  return Array.from(new Uint8Array(buf)).map(b => b.toString(16).padStart(2, '0')).join('')
}

export const hashSHA1   = t => sha('SHA-1', t)
export const hashSHA256 = t => sha('SHA-256', t)
export const hashSHA512 = t => sha('SHA-512', t)
